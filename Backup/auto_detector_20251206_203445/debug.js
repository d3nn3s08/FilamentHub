            ...
            try {
                ...
                if (result.success) {
                    if (result.message === 'Datenbankspalte bereits vorhanden') {
                        ...
                        if (migrateOutput) migrateOutput.innerHTML = '🟢 Übersprungen, da bereits vorhanden.';
                    } else {
                ...
                if (migrateOutput) migrateOutput.innerHTML = '⚠️ Migration Fehler: ' + err.message;
            }
...
function setupDbEditorListeners() {
    ...
    executeBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.success) {
                outputBox.textContent = data.message || 'Erfolgreich ausgeführt.';
                if (data.data) {
...
function setupConfigEditor() {
    ...
    editor.addEventListener('input', () => {
        ...
        if (isModified) {
            status.textContent = '✏️ Nicht gespeichert';
        ...
        if (data.available) {
            container.innerHTML = `
                ...
                <div class="info-item">
                    ...
                    <span class="value">${data.compose_available ? '✔️ Ja' : '❌ Nein'}</span>
                </div>
            `;
        } else {
            container.innerHTML = `
...
function setupServiceListeners() {
    ...
    document.getElementById('listOutdated').addEventListener('click', async () => {
        ...
        if (result.packages && result.packages.length > 0) {
            ...
            output.classList.add('show');
            showNotification(` ${result.count} Updates verfügbar!`, 'warning');
        } else {
...
function setupDatabaseListeners() {
    ...
    if (migrateBtn) {
        migrateBtn.addEventListener('click', async () => {
            ...
            if (statusInitial) {
                statusInitial.textContent = '⏳ Migration läuft...';
                statusInitial.style.color = 'var(--accent)';
            }
            if (statusSkipped) {
                statusSkipped.textContent = '⏭️ Übersprungen (da vorhanden)';
            ...
            try {
                ...
                if (statusInitial) {
                    if (data.message && data.message.includes('bereits durchgeführt')) {
                        statusInitial.textContent = '✔️ Bereits durchgeführt';
                        statusInitial.style.color = 'var(--success)';
                    } else if (data.message && (
                        data.message.toLowerCase().includes('übersprungen') ||
                        data.message.toLowerCase().includes('bereits vorhanden') ||
                        data.message.toLowerCase().includes('keine änderungen')
                    )) {
                        statusInitial.textContent = '⏭️ Übersprungen (da vorhanden)';
                        statusInitial.style.color = 'var(--accent)';
                        if (statusSkipped) {
    ...
    if (linkCard) {
        window.addCustomLink = function() {
            ...
            if (!url) return;
            const label = prompt('Label für den Link:');
            if (!label) return;
...
async function customScan() {
    ...
    // Host-Anzahl abschätzen (nur IPv4), max 254 wie Backend
    ...
    resultsDiv.innerHTML = '';
    progressDiv.innerHTML = `
        <div>Custom Scan läuft... (${ipRange} | Ports: ${ports.join(', ')} | ~${hostCount} Hosts)</div>
        <div class="progress-bar">
            <div class="progress-fill" style="width: 0%"></div>
        </div>
    `;
        ...
        <div class="modal-content" style="max-width:520px;">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <small style="color: var(--text-dim);">Für Bambu Cloud (optional, aber vom Backend gefordert)</small>
                </div>
                <div class="form-group">
...
let currentMqttPrinter = null; // aktuell ausgewählter Drucker für MQTT
let lastMqttRejected = false; // merkt letzten Verbindungsfehler
// Speichere letzte Nachricht pro Topic für den JSON-Inspector
...
const RATE_WINDOW = 60; // 60 Sekunden für Rate-Berechnung
...
async function loadMqttStatus() {
    try {
        ...
        // Wichtig: Top-Card IDs verwenden, Live-Rohdaten-IDs NICHT überschreiben
...
function setupMqttListeners() {
    ...
    document.querySelectorAll('.topic-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            // Seriennummer aus ausgewähltem MQTT-Drucker
...
async function publishMessage() {
    ...
    try {
        ...
        const data = await response.json();
        if (data.success) {
            showNotification('Nachricht veröffentlicht!', 'success');
        } else {
...
let rawPretty = true;  // Immer hübsche JSON-Darstellung
...
function connectRawMqttWebSocket() {
    ...
    rawMqttWebSocket.onopen = () => {
        ...
        rawFlushTimer = setInterval(() => {
            ...
            rawBufferLast = null; // nach Anzeige zurücksetzen
...
// Eigene Pause-Variable für Live Raw Output
...
function addToLiveRawOutput(text) {
    ...
    // Entferne Platzhalter nur bei grünem "AN"-Text
...
// Filter-Funktion für Live Raw Output
...
window.addEventListener('unload', () => {
    if (mqttConnected) {
        ...
        if (brokerElem && portElem) {
            ...
            // Sende Disconnect-Request synchron (damit er vor dem Unload ausgeführt wird)
...
function updateTopicList(topics) {
    ...
    topics.forEach(topic => {
        ...
        `;
        // Klick auf Topic öffnet JSON-Inspector (letzte bekannte Nachricht)
...
function displayMqttMessage(message) {
    ...
    // Zusätzlich: Spiegle jede eingehende Nachricht in den Raw-Output
    ...
    // Speichere letzte Nachricht dieses Topics für den Inspector
    if (message.topic) {
...
function showInspectorForTopic(topic) {
    ...
    if (!entry) {
        ...
        tree.innerHTML = `<p style="color: var(--text-dim); text-align:center; padding:8px;">Keine Daten für dieses Topic vorhanden.</p>`;
        return;
    }

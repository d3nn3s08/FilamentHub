...
from app.routes.admin_routes import audit, client_ip
...
def run_coverage(request: Request):
    ...
    try:
        ...
        try:
            ...
            if index_file.exists():
                ...
                if overall is not None:
                    ...
                    try:
                        ...
                        _error_logger.exception("Failed to read coverage history file %s", hist_file)
                        arr = []
                    arr.append(entry)
                    arr = arr[-200:]
            ...
            _error_logger.exception("Failed to delete temp coverage DB %s", temp_db)
...
def coverage_history(request: Request):
    """Return stored coverage history as JSON (PUBLIC - für Charts im UI)."""
    # ✅ NO admin_required - nur Anzeige, keine Änderung

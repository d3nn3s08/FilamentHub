...
def get_setting(session: Session, key: str, default: str | None = None) -> str | None:
    ...
    if default is not None:
        # Auto-init missing setting with default
...
async def update_settings(payload: dict, session: Session = Depends(get_session)):
    allowed_keys = {"ams_mode", "debug_ws_logging", "debug_center_mode"}
    if not any(k in payload for k in allowed_keys):
        raise HTTPException(status_code=400, detail="Keine gültigen Settings übergeben.")

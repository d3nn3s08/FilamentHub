...
def _is_admin(request: Request | None) -> bool:
    ...
    try:
        ...
        logger.exception("Admin token lookup failed")
        return False
...
async def debug_logs(
    ...
    try:
        ...
        logger.exception("Log access denied for module=%s", module)
        raise HTTPException(status_code=403, detail=str(exc)) from exc
    except ValueError as exc:
        logger.exception("Invalid log query for module=%s", module)
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except Exception as exc:
        logger.exception("Unexpected log query failure for module=%s", module)

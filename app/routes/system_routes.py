...
def save_config(config: dict) -> None:
    try:
        ...
        try:
            ...
            logging.getLogger("app").exception("Failed to resolve config save caller")
            caller = "unknown"
        logger.info(f"Writing config.yaml (system_routes.save_config) called from {caller}")
    except Exception:
        logging.getLogger("app").exception("Failed to log config save call")
...
def system_status():
    ...
    try:
        ...
        logger.exception("Failed to load config settings from database")
        cfg_settings = None
            ...
            for p in printers:
                ...
                if p.printer_type in ["bambu", "bambu_lab"]:
                    ...
                    try:
                        ...
                        logger.exception("Failed to probe Bambu printer connectivity for %s", p.ip_address)
                if p.printer_type == "klipper":
                    port = p.port or 7125
                    ...
                    try:
                        ...
                        logger.exception("Failed to probe Klipper printer connectivity for %s", url)
    except Exception:
        logger.exception("Failed to load active printers for system status")
    printer_info = {
        "bambu": bambu_status,

...
def run_command(command: str, cwd: Optional[str] = None, shell: bool = True, env: Optional[dict] = None) -> CommandResult:
    ...
    try:
        ...
        logger.exception("Command timeout running: %s", command)
        return CommandResult(
            success=False,
...
def list_python_processes():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'memory_info']):
        try:
            ...
            logger.exception("Failed to inspect process during process list")
            continue
...
def list_dependencies():
    ...
    if result.success and result.output:
        ...
        try:
            ...
            logger.exception("Failed to parse pip list output")
...
def list_outdated_dependencies():
    ...
    if result.success and result.output:
        ...
        try:
            ...
            logger.exception("Failed to parse outdated pip list output")
...
async def update_all_dependencies():
    ...
    try:
        ...
        logger.exception("Failed to update all dependencies")
        return CommandResult(
            success=False,
...
async def run_tests():
    ...
    try:
        ...
        logger.exception("Basic test run failed")
        return create_test_response(
            status="blocked",
...
async def run_tests_with_coverage():
    ...
    try:
        ...
        logger.exception("Coverage test run failed")
        return create_test_response(
            status="blocked",
...
async def run_smoke_tests():
    ...
    try:
        ...
        logger.exception("Smoke CRUD test run failed")
        return create_test_response(
            status="blocked",
...
async def run_db_tests():
    ...
    try:
        ...
        logger.exception("DB CRUD test run failed")
        return create_test_response(
            status="blocked",
            message="Test konnte nicht ausgeführt werden",
            details=str(e)
...
async def run_all_tests():
    ...
    try:
        ...
        logger.exception("Test suite run failed")
        return create_test_response(
            status="blocked",
            message="Tests konnten nicht ausgeführt werden",
            details=str(e)
...
async def create_backup():
    ...
    if files_added == 0:
        try:
            ...
            logger.exception("Failed to remove empty backup archive %s", zip_path)
...
def get_server_stats():
    ...
    try:
        ...
        logger.exception("Failed to read process connections")
        connections = 0
    ...
    try:
        ...
        logger.exception("Failed to determine hostname: %s", exc)
        hostname = "Unknown"

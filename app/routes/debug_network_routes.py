...
def _private_ipv4_from_psutil() -> Optional[str]:
    try:
        ...
        logger.exception("Failed to resolve private IPv4 from psutil")
        return None
    return None
...
def _suggest_range(ip: str) -> Optional[str]:
    ...
    try:
        ...
        logger.exception("Failed to build suggested range from ip=%s", ip)
        return None
...
def _detect_local_ip() -> Optional[str]:
    try:
        ...
        logger.exception("Failed to detect local IP via UDP probe")
...
async def get_network_info():
    ...
    try:
        ...
        logger.exception("Failed to read hostname")
        hostname = None

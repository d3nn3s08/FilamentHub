...
def _validate_ipv4(ip: str) -> str:
    try:
        ...
        log.exception("Invalid IPv4 address provided: %s", ip)
        raise HTTPException(status_code=400, detail="ip must be ipv4")
...
async def check_port(ip: str, port: int, timeout: float = 0.3) -> bool:
    ...
    try:
        ...
        log.exception("check_port failed for %s:%s -> %s", ip, port, exc)
        return False
...
def get_hostname(ip: str) -> Optional[str]:
    ...
    try:
        ...
        log.exception("get_hostname failed for %s: %s", ip, exc)
        return None
...
async def scan_network(request: ScanRequest):
    ...
    try:
        ...
        log.exception("Network scan failed for ip_range=%s", request.ip_range)
...
async def quick_scan():
    ...
    try:
        ...
        log.exception("Failed to build quick scan IP list from local subnet: %s", exc)
        # Breites Fallback: 192.168.x.x (Heimnetz), 10.x.x.x (Unraid/TrueNAS), 172.16.x.x (NAS-Bridges)
        for prefix in [
            "192.168.0", "192.168.1", "192.168.2", "192.168.178",
...
async def detect_bambu_printers():
    ...
    try:
        ...
        log.exception("Failed to determine local IP for detect_bambu_printers (fallback): %s", exc)
        # Fallback
...
async def detect_klipper_printers():
    ...
    try:
        ...
        log.exception("Failed to determine local IP for detect_klipper_printers: %s", exc)
        network_ranges = ["192.168.0.0/24", "192.168.1.0/24", "192.168.178.0/24"]
...
def get_network_info():
    ...
    try:
        ...
        log.exception("Failed to determine local IP in get_network_info: %s", exc)
        local_ip = "127.0.0.1"
...
async def test_printer_port(payload: PrinterTestRequest):
    ...
    try:
        ...
        log.exception("Port test timeout ip=%s port=%s", ip, port)
        return {
            "ok": True,
            "ip": ip,
            "port": port,
        ...
        log.exception("Port test refused ip=%s port=%s", ip, port)
        return {
            "ok": True,
            "ip": ip,
            "port": port,
        ...
        log.exception("Port test OS error ip=%s port=%s", ip, port)
        return {
            "ok": True,
            "ip": ip,
            "port": port,
        ...
        detail = str(exc)[:120]
        log.exception("Port test exception ip=%s port=%s err=%s", ip, port, detail)
        return {
            "ok": False,
            "ip": ip,
            "port": port,
...
async def probe_printer(payload: ProbeRequest):
    host = _validate_ipv4((payload.host or "").strip())
    port = payload.port
    if not (1 <= port <= 65535):
        raise HTTPException(status_code=400, detail="port must be between 1 and 65535")
    timeout_s = 2.0
    ...
    try:
        with socket.create_connection((host, port), timeout=timeout_s):
            ...
            if port == 7125:
                try:
                    ...
                    log.exception("Probe HTTP check failed host=%s port=%s", host, port)
                    http_status = None
            else:
                # TCP-Connect erfolgreich -> als OK markieren
                http_status = 200
    except socket.timeout:
        log.exception("Probe timeout host=%s port=%s", host, port)
        return {
            "ok": True,
            "status": "FEHLER",
            "latency_ms": None,
        ...
        log.exception("Probe refused host=%s port=%s", host, port)
        return {
            "ok": True,
            "status": "FEHLER",
            "latency_ms": None,
        ...
        log.exception("Probe OS error host=%s port=%s", host, port)
        return {
            "ok": True,
            "status": "FEHLER",
            "latency_ms": None,
        ...
        detail = str(exc)[:120]
        log.exception("Probe exception host=%s port=%s err=%s", host, port, detail)
...
def _fingerprint_port(host: str, port: int, timeout_s: float):
    ...
    try:
        ...
        log.exception("Fingerprint timeout host=%s port=%s", host, port)
        return {
            "reachable": False,
            "error_class": "timeout",
        ...
        log.exception("Fingerprint refused host=%s port=%s", host, port)
        return {
            "reachable": False,
            "error_class": "refused",
        ...
        log.exception("Fingerprint OS error host=%s port=%s", host, port)
        return {
            "reachable": False,
        ...
        detail = str(exc)[:120]
        log.exception("Fingerprint exception host=%s port=%s err=%s", host, port, detail)
        return {
            "reachable": False,
            "error_class": "error",
            "message": detail,
...
def _get_int(settings: Dict[str, str], key: str, default: int) -> int:
    ...
    try:
        ...
        log.exception("Invalid int setting key=%s value=%s; using default=%s", key, val, default)
        return default
...
def _get_ports(settings: Dict[str, str], key: str, default_ports: List[int]) -> List[int]:
    ...
    if isinstance(val, str) and val.strip().startswith("["):
        try:
            ...
            log.exception("Failed to parse ports JSON for key=%s value=%s", key, val)
            raw_val = val
    ...
    for p in parts:
        try:
            ...
            log.exception("Invalid port value in settings key=%s value=%s", key, p)
            continue

...
# Admin Panel (öffentlich zugänglich - kein Auth erforderlich)
...
# --- Audit Logger ---
_audit_logger = logging.getLogger("app")
...
def audit(event: str, details: dict):
    try:
        ...
        _error_logger.exception("Failed to write audit log event=%s", event)

def client_ip(request: Request) -> str:
    forwarded = request.headers.get("x-forwarded-for")
    if forwarded:
        # Take only the first entry; the header can be comma-separated
...
def is_token_active(token: Optional[str]) -> bool:
    ...
    if expiry <= int(time.time()):
        ...
        try:
            ...
            _error_logger.exception("Failed to delete expired admin token")
        return False
    return True
...
def run_migration(request: Request, _: None = Depends(admin_required)):
...
async def delete_entry(request: Request, _: None = Depends(admin_required), session: Session = Depends(get_session)):
...
# ============================================================
# ADMIN: PRINTER RUNTIME MODEL RESET
#
# Diese Aktion ist notwendig fuer Drucker, die vor Einfuehrung
# der Serien-Logik (A / X / P / H) angelegt wurden.
#
# Die Serie ist die einzige Quelle der Wahrheit.
# Das Runtime-Modell wird nach Server-Neustart neu initialisiert.
#
# ============================================================
@router.post("/api/admin/printers/{printer_id}/reset-runtime-model")
def reset_printer_runtime_model(
    printer_id: str,
    request: Request,
    ...
    if not printer:
        audit("admin_printer_runtime_reset_not_found", {"ip": client_ip(request), "printer_id": printer_id})
        raise HTTPException(status_code=404, detail="Printer not found")

    # NUR Runtime-/Cache-Felder zuruecksetzen (serie bleibt unveraendert)
    if hasattr(printer, "model"):
        setattr(printer, "model", None)
    if hasattr(printer, "runtime_model"):
        setattr(printer, "runtime_model", None)
    if hasattr(printer, "detected_model"):
        setattr(printer, "detected_model", None)
    if hasattr(printer, "last_model_source"):
        setattr(printer, "last_model_source", None)

    session.add(printer)
    session.commit()
    audit("admin_printer_runtime_reset", {"ip": client_ip(request), "printer_id": printer_id})
    return {"success": True}


# Admin Notifications Seite (öffentlich zugänglich)
    ...
    if verify_admin_password(password):
        ...
        if ip in failed_logins:
            try:
                ...
                _error_logger.exception("Failed to clear failed login entry for ip=%s", ip)
...
async def admin_logout(request: Request):
    ...
    if token and token in admin_tokens:
        try:
            ...
            _error_logger.exception("Failed to delete admin token on logout")
    response = JSONResponse({"success": True})
...
async def set_greeting_text(request: Request, _: None = Depends(admin_required), session: Session = Depends(get_session)):
    data = await request.json()
...
def reset_pro_mode_warning(request: Request, _: None = Depends(admin_required), session: Session = Depends(get_session)):

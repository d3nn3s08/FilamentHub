...
async def lifespan(app: FastAPI):
    ...
    # Netzwerk-Modus prüfen: Bridge-Modus verhindert Drucker-Scanner
    import socket as _socket
    try:
        _s = _socket.socket(_socket.AF_INET, _socket.SOCK_DGRAM)
        _s.connect(("8.8.8.8", 80))
        _own_ip = _s.getsockname()[0]
        _s.close()
        _parts = _own_ip.split(".")
        if len(_parts) == 4 and _parts[0] == "172" and 17 <= int(_parts[1]) <= 31:
            logger.warning(
                "[NETWORK] Container laeuft im Docker Bridge-Modus (IP: %s). "
                "Der Drucker-Scanner kann das LAN NICHT erreichen. "
                "Bitte 'network_mode: host' in docker-compose.yml setzen.",
                _own_ip
            )
        else:
            logger.info("[NETWORK] Netzwerk-Modus OK (eigene IP: %s)", _own_ip)
    except Exception:
        pass
        ...
        while not _shutdown_reconnect.is_set():
            try:
                ...
                for printer in printers:
                    ...
                    try:
                        success = startup_connect_printer(printer)
                        if success:
    ...
    # [BETA] Klipper-Support: Klipper Polling-Task starten (1s HTTP-Poll gegen Moonraker)
    klipper_poller_task = None
    try:
        from services.klipper_polling_service import run_klipper_poller
        klipper_poller_task = asyncio.create_task(
            run_klipper_poller(printer_service=getattr(app.state, "printer_service", None))
        )
        logger.info("[APP] Klipper Poller Background-Task gestartet")
    except Exception:
        logger.exception("[APP] Klipper Poller konnte nicht gestartet werden")
    ...
    try:
        ...
        # [BETA] Klipper-Support: Klipper Poller stoppen
        try:
            from services.klipper_polling_service import stop_klipper_poller
            stop_klipper_poller()
            if klipper_poller_task and not klipper_poller_task.done():
                klipper_poller_task.cancel()
                try:
                    await klipper_poller_task
                except asyncio.CancelledError:
                    pass
            logger.info("[APP] Klipper Poller gestoppt")
        except Exception:
            logger.exception("[APP] Klipper Poller Shutdown Fehler")

...
function toggleBambuCredentials() {
  ...
  const bambuSeries = $('manualBambuSeries');
  const portInput = $('manualPort');

  if (!typeSelect || !bambuCreds || !bambuAccess || !bambuSeries) return;
  ...
  bambuSeries.style.display = isBambu ? 'block' : 'none';
  if (!isBambu) {
    clearSeriesSelection('manualSeries');
  }
...
function toggleScannerBambuCredentials() {
  ...
  const bambuSeries = $('scannerBambuSeries');
  ...
  bambuSeries.style.display = isBambu ? 'block' : 'none';
  if (!isBambu) {
    clearSeriesSelection('scannerSeries');
  }
}

function getSelectedSeriesValue(groupName) {
  const selected = document.querySelector(`input[name="${groupName}"]:checked`);
  return selected ? selected.value : '';
}

function clearSeriesSelection(groupName) {
...
async function handleScannerConfirm() {
  ...
  const series = getSelectedSeriesValue('scannerSeries');
  ...
  if (isBambu) {
    ...
    if (!series) {
      if (saveResult) {
        ...
        saveResult.textContent = 'Bitte Bambu-Serie (A/X/P/H) waehlen';
      }
      return;
    }
...
async function handleManualSave() {
  ...
  const series = getSelectedSeriesValue('manualSeries');
  ...
  if (printerType === 'bambu') {
    if (!series) {
      testResult.style.display = 'block';
      testResult.className = 'test-result test-result-error';
      testResult.textContent = 'Bitte Bambu-Serie (A/X/P/H) waehlen';
      saveBtn.disabled = false;
...
function updateLivePayloadUI(state) {
  ...
  const lastSeen = state.last_seen || state.ts || null;
  const printerOnline = state.printer_online === true;
  const autoConnect = state.auto_connect_enabled === true;
  const mqttConnected = state.mqtt_connected === true;
  const cacheAgeSec = state.cache_age_sec;
  const offlineReason = state.offline_reason || null;

  setText('liveDeviceName', device);

  const statusParts = [
    autoConnect ? 'Auto-Connect aktiv' : 'Auto-Connect aus',
    mqttConnected ? 'Broker verbunden' : 'Broker getrennt',
    printerOnline ? 'Drucker online' : 'Drucker offline',
  ];
  setText('liveStatus', statusParts.join(' – '));

  if (cacheAgeSec != null) {
    const ageText = formatAgeSeconds(cacheAgeSec);
    const reasonText = printerOnline ? '' : (offlineReason ? ` (${offlineReason})` : '');
    const el = document.getElementById('liveLastUpdate');
    if (el) {
      el.textContent = `Letzte Nachricht vor ${ageText}${reasonText}`;
      el.title = lastSeen || '';
    }
  } else {
    const reasonText = offlineReason ? ` (${offlineReason})` : '';
    setText('liveLastUpdate', `Letzte Nachricht: nie${reasonText}`);
  }

...
function applyDebugMode(mode) {
  ...
  document.querySelectorAll('[data-mode="lite"]').forEach(el => {
    el.style.display = '';
  });
  const liteBtn = $('#debugModeLite');
  const proBtn = $('#debugModePro');
  liteBtn?.classList.toggle('active', normalized === 'lite');
  proBtn?.classList.toggle('active', normalized === 'pro');
  const label = $('#debugModeLabel');
  if (label) label.textContent = 'Mode: ' + (normalized === 'pro' ? 'Pro' : 'Lite');
}

function initModeButtons() {
  $('#debugModeLite')?.addEventListener('click', () => applyDebugMode('lite'));
  $('#debugModePro')?.addEventListener('click', () => applyDebugMode('pro'));
}

...
class UniversalMapper:
    ...
    def map(self, data: Dict[str, Any]) -> PrinterData:
        ...
        try:
            ...
            logger.exception("Failed to map printer payload; returning best-effort data")
        return pd
    ...
    def _detect_model(self, data: Dict[str, Any]) -> Optional[str]:
        # A1/A1 MINI: Erkenne als A1MINI wenn EINZIGES Top-Level Field "print" ist
        # und "print" enthält die erwarteten Fields
        if "print" in data and len(data) == 1:
            print_obj = data.get("print", {})
            if isinstance(print_obj, dict):
                # A1 Mini hat diese spezifischen Fields unter print
                if any(k in print_obj for k in ["gcode_state", "mc_percent", "mc_remaining_time", "subtask_name"]):
                    return "A1MINI"
        
        # X1C/P1 haben Top-Level gcode_state, mc_percent, lights_report
        # (Fall 1: Klassischer Report mit Top-Level Fields)
        if "gcode_state" in data or "mc_percent" in data or "lights_report" in data:
            return "X1C"
        
        # Fall 2: Nur ams/vt_tray/mc_print (seltener, aber auch X1C)
        if "mc_print" in data or ("ams" in data and len(data) > 1) or "vt_tray" in data:
            return "X1C"
    ...
    def _map_bambu(self, data: Dict[str, Any], out: PrinterData) -> None:
        ...
        # TEMPERATURE (X1C hat teilweise Top-Level, A1 Mini immer unter 'print')
        nozzle = (
            data.get("nozzle_temper")
            or data.get("nozzle_temp")
            or data.get("extruder_temp")
            or pr.get("nozzle_temper") if isinstance(pr, dict) else None
            or pr.get("nozzle_temp") if isinstance(pr, dict) else None
        ...
        bed = (
            data.get("bed_temper")
            or pr.get("bed_temper") if isinstance(pr, dict) else None
            or pr.get("bed_temp") if isinstance(pr, dict) else None
            or self._deep_get(data, ["device", "bed", "info", "temp"])
        )
        chamber = (
            data.get("chamber_temper")
            or pr.get("chamber_temper") if isinstance(pr, dict) else None
            or self._deep_get(data, ["device", "ctc", "info", "temp"])
        )
        ...
        current_layer = (
            data.get("layer_num")
            or pr.get("layer_num") if isinstance(pr, dict) else None
            or self._deep_get(pr, ["layer_num"])
            or self._deep_get(data, ["print", "3D", "layer_num"])
        )
        total_layer = (
            data.get("total_layer_num")
            or pr.get("total_layer_num") if isinstance(pr, dict) else None
        ...
        try:
            ...
            logger.exception("Failed to parse AMS data in Bambu mapper; using empty AMS list")
            ams_units = []
        ...
        # JOB (A1 Mini sendet gcode_file unter 'print')
        job_block = data.get("job") or pr.get("job") if isinstance(pr, dict) else {}
        self._set(out.job, "file", data.get("gcode_file") or data.get("file") or pr.get("gcode_file") if isinstance(pr, dict) else None)
        self._set(out.job, "time_remaining", self._first_defined_float([
            data.get("mc_remaining_time"),
            data.get("remain_time"),
            pr.get("mc_remaining_time") if isinstance(pr, dict) else None,
        ...
        # TASK ID & SUBTASK NAME (wichtig für Cloud MQTT Job-Tracking)
        task_id = (
            data.get("task_id")
            or pr.get("task_id") if isinstance(pr, dict) else None
        )
        self._set(out.job, "task_id", task_id)

        subtask_name = (
            data.get("subtask_name")
            or pr.get("subtask_name") if isinstance(pr, dict) else None
        )
        self._set(out.job, "subtask_name", subtask_name)

        gcode_file = (
            data.get("gcode_file")
            or pr.get("gcode_file") if isinstance(pr, dict) else None
        )
        self._set(out.job, "gcode_file", gcode_file)
    ...
    def _map_klipper(self, data: Dict[str, Any], out: PrinterData) -> None:
        ...
        if fan_speed is not None:
            try:
                ...
                logger.exception("Failed to parse klipper fan speed value=%s; setting to None", fan_speed)
    ...
    def _map_generic(self, data: Dict[str, Any], out: PrinterData) -> None:
        try:
            ...
            logger.exception("Failed to copy generic payload into extra; leaving empty")
            out.extra = {}
    ...
    def _safe_int(self, val: Any) -> Optional[int]:
        try:
            ...
            logger.exception("Failed to cast value to int: %s", val)
            return None
    ...
    def _safe_float(self, val: Any) -> Optional[float]:
        try:
            ...
            logger.exception("Failed to cast value to float: %s", val)
            return None

    def _deep_get(self, data: Any, path: list, default=None):
        """
        Safely navigate nested dict/list structure.
        Returns default if path not found (normal behavior - no error logging).
        """
        cur = data
        try:
            for p in path:
            ...
            # Path not found - this is normal, return default silently
            return default
        except Exception:
            # Unexpected error - log this
            logger.exception("Unexpected error in deep-get path=%s; returning default", path)
            return default
    ...
    def _get_setting_value(self, key: str, default: Optional[str] = None) -> Optional[str]:
        """
        Minimaler Settings-Reader für Mapper-Kontext ohne Request/Dependency.
        ...
        try:
            ...
            logger.exception("Failed to read setting key=%s; using default", key)
            return default
    ...
    def map_ams_block(self, data: Dict[str, Any], out: PrinterData) -> None:
        ...
        try:
            ...
            logger.exception("Failed to parse AMS data in map_ams_block; using empty AMS list")
            ams_units = []

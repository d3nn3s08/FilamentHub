...
def _load_mqtt_logging_config() -> dict:
    ...
    try:
        ...
        logging.getLogger("mqtt").exception("Failed to load mqtt_logging config; using defaults")
...
# ============================================================
# MULTI-PRINTER SUPPORT: Dict[printer_id, instance/state]
# ============================================================
_clients_lock = Lock()
_client_instances: Dict[str, PrinterMQTTClient] = {}  # printer_id -> client
_client_configs: Dict[str, _RuntimeConfig] = {}  # printer_id -> config
_connected_since: Dict[str, datetime] = {}  # printer_id -> connected timestamp

# Runtime state per printer (exposed to status endpoint)
_runtime_states_lock = Lock()
_runtime_states: Dict[str, Dict[str, Any]] = {}  # printer_id -> state dict

# Legacy single-instance variables for backward compatibility with UI
_runtime_state: Dict[str, Any] = {}  # aggregated state for status() endpoint
_client_instance: Optional[PrinterMQTTClient] = None  # first connected client (for legacy API)
_client_config: Optional[_RuntimeConfig] = None  # config for legacy single client
...
# Live message buffer (last 50 messages for UI display) - global across all printers
_messages_lock = Lock()
...
def _check_start_trigger(payload: str) -> bool:
    ...
    if trigger_type == "command":
        ...
        try:
            ...
            logging.getLogger("mqtt").exception("Invalid smart logging temperature trigger value: %s", trigger_value)
        ...
        try:
            ...
            logging.getLogger("mqtt").exception("Invalid smart logging humidity trigger value: %s", trigger_value)
    return False
...
def _extract_ams_climate(payload: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logging.getLogger("mqtt").exception("Failed to parse AMS climate payload; returning None")
        return None
...
def _get_ams_climate_logger():
    ...
    with _ams_climate_lock:
        ...
        if not ams_config.get("enabled", True):
            return None
        _ams_climate_logger = logging.getLogger("mqtt")
        return _ams_climate_logger
...
def _log_ams_climate(payload: str):
    ...
    try:
        ...
        logging.getLogger("mqtt").exception("Failed to log AMS climate payload")
...
def _get_full_payload_logger():
    ...
    with _full_payload_lock:
        ...
        if not limits.get("full_payload_enabled", False):
            return None
        _full_payload_logger = logging.getLogger("mqtt")
        return _full_payload_logger
...
def _write_full_payload(topic: str, payload: str):
    ...
    try:
        ...
        logging.getLogger("mqtt").exception("Failed to write full MQTT payload log entry")
# ===================================================================
# STATUS HELPER
# ===================================================================
...
def apply_auto_connect(printer: Optional[Printer]) -> Dict[str, Any]:
    ...
    # ⚠️ WICHTIG: Während Shutdown KEINE neuen MQTT-Verbindungen starten!
    try:
        from app.main import is_app_shutting_down
        if is_app_shutting_down():
            logger.debug("apply_auto_connect skipped - app is shutting down")
            return {"success": False, "message": "app is shutting down"}
    except Exception:
        logger.exception("Failed to check shutdown state")
...
def _format_uptime(connected_since: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        logging.getLogger("mqtt").exception("Failed to format uptime value")
        return None
...
def _is_connected(client: PrinterMQTTClient) -> bool:
    try:
        ...
        logging.getLogger("mqtt").exception("Failed to check MQTT client connection state")
...
def _disconnect_printer(printer_id: str) -> None:
    """Disconnect a specific printer by ID (for multi-printer support)."""
    with _clients_lock:
        client = _client_instances.pop(printer_id, None)
        _client_configs.pop(printer_id, None)
        _connected_since.pop(printer_id, None)
    
    with _runtime_states_lock:
        if printer_id in _runtime_states:
            _runtime_states[printer_id]["connected"] = False
    ...
    if client is not None:
        try:
            ...
            logging.getLogger("mqtt").exception(f"Failed to disconnect printer {printer_id}")


def connect(config: Dict[str, Any]) -> Dict[str, Any]:
    """Connect a printer MQTT client (multi-printer support).

    Expected config keys:
    - host (str)
    - printer_id (str) - REQUIRED for multi-printer
    - cloud_serial (str) - optional
    ...
    - protocol (str, default '311')
    - model (str, default 'X1C')
    """
    global _client_instance, _client_config, _transport_connected_since, _runtime_state
    try:
        _reset_topic_stats()
        ...
        # CRITICAL: printer_id is now required for multi-printer
        printer_id = config.get("printer_id")
        if not printer_id:
            return {"success": False, "error": "printer_id is required for multi-printer support"}

        port_raw = config.get("port", 8883)
        try:
            port = int(port_raw)
        except Exception:
            logging.getLogger("mqtt").exception("Invalid MQTT port value %s; using default 8883", port_raw)
            port = 8883
        ...
        printer_name = config.get("printer_name") or f"Printer_{printer_id[:8]}"
        model = str(config.get("model") or "X1C")
        protocol = str(config.get("protocol") or "311")
        
        # Generate unique client_id per printer
        client_id = f"filamenthub_{printer_name.replace(' ', '_')}_{printer_id[:6]}"

        # 1) Check if this printer is already connected
        with _clients_lock:
            if printer_id in _client_instances and _is_connected(_client_instances[printer_id]):
                logging.getLogger("mqtt").info(f"Printer {printer_id} already connected, disconnecting first")
                _disconnect_printer(printer_id)

        # 2) Initialize runtime state for this printer
        with _runtime_states_lock:
            _runtime_states[printer_id] = {
                "connected": False,
                "cloud_serial": cloud_serial,
        ...
        # Register printer in PrinterService
        if cloud_serial:
            try:
                ...
                logging.getLogger("mqtt").exception(
                    "Failed to register printer in PrinterService for cloud_serial=%s", cloud_serial
                )

        # Track default subscription
        if cloud_serial:
            try:
                register_subscription(f"device/{cloud_serial}/report")
            except Exception:
                logging.getLogger("mqtt").exception(
                    "Failed to register subscription for cloud_serial=%s", cloud_serial
                )
        ...
        try:
            ...
            if inner is not None:
                    ...
                    try:
                        ...
                        if isinstance(payload, bytes):
                            try:
                                ...
                                logging.getLogger("mqtt").exception("Failed to decode MQTT payload bytes; using str fallback")
                                payload = str(payload)
                        ...
                        if _should_log_message(topic, payload):
                            ...
                            try:
                                ...
                                logging.getLogger("mqtt").exception("Failed to write MQTT truncated payload log entry")
                            # Write to full payload file if enabled
                            if full_payload_enabled:
                                _write_full_payload(topic, payload)
                    except Exception:
                        logging.getLogger("mqtt").exception("Failed to process MQTT message for smart logging")
                                ...
                                if topic.endswith("/report"):
                                    try:
                                        ...
                                        try:
                                            ...
                                            logger = logging.getLogger("mqtt")
                                            logger.exception("[AMS SYNC] Failed")
                                        ...
                                        logging.getLogger("mqtt").exception("set_live_state failed")
                            except Exception:
                                logging.getLogger("mqtt").exception("Failed to update runtime state from MQTT message")
                    except Exception:
                        logging.getLogger("mqtt").exception("Unhandled exception in MQTT on_message wrapper")
                ...
                try:
                    ...
                    logging.getLogger("mqtt").exception("Failed to set MQTT client user data")
        except Exception:
            # Never fail connect due to stats wiring
            logging.getLogger("mqtt").exception("Failed to wire MQTT client message hooks")

        # 3) Connect
        print(f"[MQTT] CONNECT host={host} port={port} tls={tls} user={username}")
        try:
            logging.getLogger("mqtt").info(f"connect host={host} port={port} tls={tls} client_id={client_id}")
        except Exception:
            logging.getLogger("mqtt").exception("Failed to log MQTT connect attempt")
        client.connect()
        ...
        # 5) Wait (max 15s) until the underlying paho client is actually connected.
        ...
        while time.monotonic() < deadline:
            if _is_connected(client):
                ...
                try:
                    ...
                    logging.getLogger("mqtt").exception("Failed to update runtime state after transport connect")
                try:
                    logging.getLogger("mqtt").info(f"connected host={host} client_id={client_id}")
                except Exception:
                    logging.getLogger("mqtt").exception("Failed to log MQTT connected status")
                return {
                    "success": True,
                    "connected": True,
                    "host": host,
        ...
        try:
            logging.getLogger("mqtt").error(f"connect timeout (15s) host={host} client_id={client_id}")
        except Exception:
            logging.getLogger("mqtt").exception("Failed to log MQTT connect timeout")
        disconnect()
        # Also ensure runtime state reflects disconnected
        try:
            _runtime_state.update({"connected": False, "connected_since": None})
        except Exception:
            logging.getLogger("mqtt").exception("Failed to mark runtime state disconnected after timeout")
        return {
            "success": False,
            "error": "connect timeout (15s) - broker unreachable, auth failed, or network unstable",
        }

    except Exception as exc:
        # 4) Exception: reset instance
        logging.getLogger("mqtt").exception("Unhandled error during MQTT runtime connect; returning failure")
        _client_instance = None
        _client_config = None
        _connected_since = None
        try:
            logging.getLogger("mqtt").exception("connect error")
        except Exception:
            logging.getLogger("mqtt").exception("Failed to log MQTT connect error")
        return {"success": False, "error": str(exc)}


def disconnect() -> Dict[str, Any]:
    """Disconnect the single runtime instance, if any."""
    global _client_instance, _client_config, _connected_since, _transport_connected_since, _runtime_state

    if _client_instance is None:
        _reset_topic_stats()
        return {"success": True, "connected": False, "note": "already disconnected"}

    # Prüfe ob ein aktiver Druck läuft bevor wir die Verbindung trennnen
    printer_id = _client_config.printer_id if _client_config else None
    printer_name = _client_config.printer_name if _client_config else None
    active_job_name = None
    active_job: Optional[Dict[str, Any]] = None
    
    if printer_id:
        try:
            from app.services.job_tracking_service import get_active_job_for_printer
            active_job = get_active_job_for_printer(printer_id)
            if active_job:
                active_job_name = active_job.get("name", "Unknown Job")
        except Exception as e:
            logging.getLogger("mqtt").debug(f"Failed to check active job: {e}")
    ...
    try:
        ...
        _client_instance = None
        _client_config = None
        _connected_since = {}  # Reset to empty dict, not None
        ...
        try:
            ...
            logging.getLogger("mqtt").exception("Failed to update runtime state on disconnect")
        
        # AMS-Spulen ins Lager verschieben wenn Drucker disconnected wird
        if printer_id:
            try:
                from app.services.ams_sync import release_printer_spools_to_storage
                released = release_printer_spools_to_storage(printer_id)
                if released > 0:
                    logging.getLogger("mqtt").info(
                        f"[MQTT DISCONNECT] {released} AMS-Spulen ins Lager verschoben (Drucker {printer_name or printer_id})"
                    )
            except Exception:
                logging.getLogger("mqtt").exception("Failed to release AMS spools on disconnect")

        # Triggere Notification wenn aktiver Druck läuft
        if active_job_name and printer_name:
            try:
                ...
                trigger_notification_sync(
                    ...
                    started_at=active_job.get("started_at", "?") if active_job else "?",
                    elapsed_minutes=active_job.get("elapsed_minutes", 0) if active_job else 0
                )
            except Exception as e:
                logging.getLogger("mqtt").debug(f"Failed to trigger disconnect notification: {e}")
        
        try:
            logging.getLogger("mqtt").info("disconnect")
        except Exception:
            logging.getLogger("mqtt").exception("Failed to log MQTT disconnect")
...
def status() -> Dict[str, Any]:
    ...
    try:
        # Shallow copy as base - ensure connected is always present
        base = dict(_runtime_state) if _runtime_state else {}
        
        # CRITICAL: Always ensure 'connected' field exists (required by API schema)
        if "connected" not in base:
        ...
        if not base.get("connected_since"):
            ...
            elif _connected_since and isinstance(_connected_since, dict):
                # _connected_since is a Dict[str, datetime] - get first value if available
                first_dt = next(iter(_connected_since.values()), None)
                if first_dt:
                    base["connected_since"] = _iso_utc(first_dt)

        # Aggregate topic/message statistics - but prefer values already in _runtime_state
        stats = _aggregate_topic_stats()
        # Only use aggregated stats if runtime_state doesn't have better values
        if not base.get("message_count"):
            base["message_count"] = stats.get("message_count", 0)
        if not base.get("subscriptions_count"):
            base["subscriptions_count"] = stats.get("subscriptions_count", 0)
        if not base.get("topics_count"):
            base["topics_count"] = stats.get("topics_count", 0)
        if not base.get("last_message_time"):
            base["last_message_time"] = stats.get("last_message_time")
        ...
        # Remove non-serializable set from response
        base.pop("subscribed_topics", None)

        return base
    except Exception as exc:
        logging.getLogger("mqtt").exception("Failed to build MQTT runtime status response")

...
def backfill_weight_history_spool_number(
    session: Session,
    tray_uuid: str,
    spool_number: int,
) -> int:
    """
    Füllt nachträglich die spool_number in weight_history-Einträgen ein.

    Wird aufgerufen wenn eine Spule erstmals eine Nummer erhält (z.B. durch
    Merge im Zuordnungs-Dialog oder manuelle Vergabe). Alle WeightHistory-
    Einträge für diese tray_uuid ohne Nummer werden mit der neuen Nummer versehen.

    Args:
        session: SQLModel Session
        tray_uuid: Die permanente UUID des AMS-Tabletts (Spule)
        spool_number: Die neu vergebene Spulennummer

    Returns:
        Anzahl aktualisierter Einträge
    """
    if not tray_uuid or not spool_number:
        return 0
    ...
    try:
        ...
        entries = session.exec(
            select(WeightHistory).where(
                WeightHistory.spool_uuid == tray_uuid,
                col(WeightHistory.spool_number).is_(None),
            )
        ).all()

        count = 0
        for entry in entries:
            entry.spool_number = spool_number
            session.add(entry)
            count += 1
        ...
        if count > 0:
            ...
            logging.getLogger("app").info(
                f"[WEIGHT HISTORY BACKFILL] {count} Eintraege fuer "
                f"tray_uuid={tray_uuid[:8]}... mit spool_number=#{spool_number} aktualisiert"
            )
        ...
        logging.getLogger("app").exception(
            f"[WEIGHT HISTORY BACKFILL] Fehler fuer tray_uuid={tray_uuid}, spool_number={spool_number}"
        )
        return 0

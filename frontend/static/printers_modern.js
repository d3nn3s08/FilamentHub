...
async function loadPrintersModern() {
    ...
    try {
        ...
        // versuche Live-State mitzuladen und nach cloud_serial (Bambu) oder klipper_{id} (Klipper) zu mappen
        ...
        container.innerHTML = data.map(p => {
            const lk = p.cloud_serial || `klipper_${p.id}`;
            return renderCard({ ...p, live: liveMap[lk]?.payload || null, live_state: liveMap[lk] || null });
        }).join("");
    } catch (err) {
        console.error(err);

...
async function loadDashboardData() {
    try {
        ...
        renderLowSpoolsList(spools.filter(s => {
            ...
            const percent = toNumber(s.remaining_percent);
            if (percent != null) {
                return percent <= 20;
            }
            if (remaining != null) {
...
function renderLowSpoolsList(spools) {
    ...
    container.innerHTML = `
        <div class="info-group">
            ${spools.map(s => {
                const remaining = toNumber(s.remaining_weight_g);
                const percentValue = toNumber(s.remaining_percent);
                const percentage = percentValue != null ? percentValue : 0;
                const remainingDisplay = remaining != null ? Math.round(remaining) : 'N/A';

                return `
                    <div class="info-item" style="flex-direction: column; align-items: flex-start;">
                        <div style="display: flex; justify-content: space-between; width: 100%; margin-bottom: 8px;">
                            <span class="info-label">${s.label || 'Spule #' + s.id.substring(0, 8)}</span>
                            <span class="info-value">${remainingDisplay}g</span>
                        </div>

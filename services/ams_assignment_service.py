...
def assign_spool_manual(spool_id: str, ams_id: str, slot: int, session=None) -> None:
    ...
    WICHTIG: weight_current wird NICHT geändert bei Verdrängung!
    """
    session = _get_session(session)

    # Clear existing spool in that slot
    # OHNE weight_current oder printer_id zu löschen!
    ...
    if existing and existing.id != spool_id:
        ...
        # weight_current bleibt erhalten!
...
def remove_spool_from_ams(spool_id: str, session=None) -> None:
    """Remove AMS assignment from a spool (clear AMS fields but PRESERVE weight_current).
    
    WICHTIG: AMS ist nur eine Live-Datenquelle, keine Eigentümerin der Spulengewichte.
    - weight_current DARF NICHT gelöscht werden
    - printer_id und ams_slot werden gelöscht
    - Spule kehrt ins Lager zurück mit bekanntem Gewicht
    """
    session = _get_session(session)
    spool = session.get(Spool, spool_id)
    if not spool:

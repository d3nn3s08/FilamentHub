...
- Datum: 2025-12-06
- Dateien: `scripts/mqtt_recorder.py`, `tests/test_mapper_live_x1c.py`, `tests/live_samples/` (erwartet), `app/services/printer_data.py`, `app/services/universal_mapper.py`, `services/mqtt_protocol_detector.py`, `app/routes/mqtt_routes.py`, `app/static/debug.js`
- Aenderung: MQTT Recorder Script hinzugefügt (TLS/Bambu, Samples speichern, Mapper validieren); Live-Mapping-Tests hinzugefügt (werden übersprungen ohne Samples); PrinterData Timestamp auf timezone-aware; UniversalMapper Nozzle-Fallback erweitert; MQTT Protokoll-Detection-Hook bei Connect integriert (auto Protocol); Debug-UI Summaries ergänzt (nur, wenn Element existiert); neue Backup erstellt.
- Roadmap-Stufe: 0.6 Erweiterung / Live-Validierung / Auto-Detection
- Legacy-Code: unverändert, Fallbacks aktiv.
